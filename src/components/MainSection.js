...
<Box px={9} pt={10} mb={30} height="100vh" key={section.sectionType} backgroundColor={section.backgroundColor.hex } color={section.textColor.hex} id="section1">
  <Box py={4} fontStyle="italic" textAlign="center" textTransform="uppercase" fontFamily="Montserrat, sans-serif" fontSize={["14px","18px"]}>
    ...
    <Box w={["95%","40%"]} py={[5, 20]} textAlign="center" fontFamily="sfuitextheavy" fontWeight={500} color="black">
      ...
      <Box py={5} textAlign="center">
        <Link href={`https://api.whatsapp.com/send?phone=${section.whatsapp}`} target="_blank" display="block" height="70px" width={["100%","488px"]} borderRadius="5px" margin="auto" boxShadow="#6a6a6a" backgroundColor={section.ctaBackgroundColor.hex} >
          <Box pt={2} lineHeight="1.1em" fontSize="28px" pb={0} textTransform="uppercase">{section.CtalineOne}</Box>
          <Box fontSize={16}>{section.CtalineTwo}</Box>
        </Link>
      </Box>
      <Box fontSize={18} fontFamily=" Montserrat, sans-serif" fontStyle="italic" fontWeight={500}>
